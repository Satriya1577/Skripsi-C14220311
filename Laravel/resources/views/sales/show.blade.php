...
<main class="max-w-7xl mx-auto px-6 py-6 space-y-8">
  ...
  <nav aria-label="breadcrumb" class="text-xs text-muted">
    <ol class="flex items-center space-x-2">
      ...
      <li><a href="{{ route('sales.index') }}" class="hover:text-blue-600 transition-colors">Sales Orders</a></li>
      <li class="opacity-40">/</li>
  ...
  <header class="flex justify-between items-end">
    <div>
      ...
      <h1 class="text-3xl font-extrabold text-slate-800 mt-1">{{ $salesOrder->so_code }}</h1>
  ...
  <form id="mainForm" action="{{ route('sales.updateStatus', $salesOrder->id) }}" method="POST">
    ...
    <section class="bg-carbonSoft rounded-xl p-6 border border-carbon space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 text-sm">
        ...
        <div class="bg-carbon rounded-lg p-4 border border-carbonSoft">
          ...
          <p class=" text-xl font-bold text-slate-800">Rp {{ number_format($salesOrder->grand_total, 2, ',', '.') }}</p>
        </div>
        <div class="bg-carbon rounded-lg p-4 border border-carbonSoft">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
          ...
          <div class="bg-carbon rounded-lg p-4 border border-carbonSoft">
            ...
            <p class="text-lg font-bold text-silver">
              ...
              @else
                <span class="text-slate-800">Ditanggung Perusahaan</span>
              @endif
            </p>
          </div>
    ...
    @if($salesOrder->status == 'draft')
      <section class="bg-carbonSoft rounded-xl p-6 border border-petronas/30 shadow-lg shadow-petronas/5 relative overflow-hidden mt-6">
        <div class="relative z-10">
          <h2 class="text-lg font-bold text-slate-800 mb-1">Add Sales Item</h2>
                ...
                <select id="productSelect" class="w-full appearance-none bg-carbon border border-muted/30 text-silver text-sm rounded-lg focus:ring-petronas focus:border-petronas block p-3 pr-10 hover:border-petronas/50 transition">
                  ...
                  @foreach($products as $product)
                    <option value="{{ $product->id }}" 
                      ...
                      {{ $product->code }} - {{ $product->name }} ({{ $product->packaging }}) (Available Stok: {{ $product->current_stock - $product->committed_stock }}) 
                    </option>
                  @endforeach
                </select>
    ...
    <section class="bg-carbonSoft rounded-xl p-6 border border-carbon mt-6">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-bold text-slate-800">Order Items List</h2>
        <span class="text-xs bg-carbon px-3 py-1 rounded-full text-slate-800 border border-carbon">Items Count: <span id="itemCountDisplay">{{ $salesOrder->items->count() }}</span></span>
      </div>
      ...
      <div class="overflow-x-auto rounded-lg border border-carbon">
        <table class="w-full text-sm">
          ...
          <tbody id="itemsTableBody" class="divide-y divide-carbon/50">
            ...
            <tr class="hover:bg-carbon transition-colors" id="row-db-{{ $item->id }}">
              <td class="px-4 py-3">
                ...
                <div class="text-xs text-muted ">{{ $item->product->code }}</div>
              </td>
              ...
              <td class="px-4 py-3 text-center">
                @if($salesOrder->status == 'draft')
                  <input type="number" 
                      ...
                      value="{{ $item->quantity }}" 
                      min="1"
                      class="w-16 p-1 bg-blackBase border border-muted/30 rounded text-center text-silver text-xs focus:ring-1 focus:ring-petronas focus:border-petronas"
                  >
                @else
                  <span class="text-silver ">{{ $item->quantity }}</span>
                @endif
              </td>
              ...
              <td class="px-4 py-3 text-center">
                @if($salesOrder->status == 'draft')
                  ...
                  >
                @else
                  <span class="text-silver ">{{ $item->discount_percent + 0 }}%</span>
                @endif
              </td>
          ...
          <tfoot class="bg-carbon/50">
            ...
            <tr>
              <td colspan="5" class="px-4 py-2 text-right text-muted text-xs uppercase">Shipping Cost <span class="text-[10px] text-slate-800 ml-1">({{ $salesOrder->shipping_payment_type == 'bill_to_customer' ? 'Customer' : 'Company' }})</span></td>
              <td class="px-4 py-2 text-right text-silver text-sm">
                @if($salesOrder->shipping_payment_type == 'bill_to_customer') Rp {{ number_format($salesOrder->shipping_cost, 2, ',', '.') }}
                @else <span class=" text-muted line-through">Rp {{ number_format($salesOrder->shipping_cost, 2, ',', '.') }}</span> <span class=" text-[10px] text-slate-800 block">Gratis Ongkir</span> @endif
              </td>
              @if($salesOrder->status == 'draft') <td></td> @endif
            </tr>
            <tr class="border-t border-carbon">
              ...
              <td class="px-4 py-4 text-right text-slate-800 font-extrabold text-lg">Rp <span id="grandTotalDisplay">{{ number_format($salesOrder->grand_total, 2, ',', '.') }}</span></td>
              @if($salesOrder->status == 'draft') <td></td> @endif
            </tr>
          </tfoot>
        </table>
...
<script>
  ...
  function addItemToTable() {
    ...
    row.innerHTML = `
      <td class="px-4 py-3">
        <div class="text-silver font-bold">${productName} <span class="text-xs text-slate-800 font-normal">(New)</span></div>
        <div class="text-xs text-muted ">${productCode}</div>
      </td>
