...
<head>
  <meta charset="UTF-8">
  <title>Payment - {{ $salesOrder->so_code }} | Production Planning System</title>
...
<main class="max-w-7xl mx-auto px-6 py-6 space-y-8">
  ...
  <header class="flex justify-between items-end">
    <div>
      ...
      <h1 class="text-3xl font-extrabold text-slate-800 mt-1">Pembayaran: {{ $salesOrder->so_code }}</h1>
  ...
  <section class="bg-carbonSoft rounded-xl p-6 border border-carbon">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
      ...
      <div class="bg-carbon rounded-lg p-4 border border-carbonSoft">
        ...
        <p class=" text-xl font-bold text-silver">Rp {{ number_format($salesOrder->grand_total, 2, ',', '.') }}</p>
      </div>
  ...
  {{-- SECTION 2: FORM PEMBAYARAN BARU --}}  
  ...
  @if($salesOrder->status == 'draft')
    <div class="bg-warning/10 border border-warning/30 rounded-xl p-6 flex items-center gap-4">
      ...
      <div>
        <h3 class="text-lg font-bold text-warning">Pesanan Masih Draft</h3>
        <p class="text-sm text-silver mt-1">
          Silakan konfirmasi pesanan (Confirm) terlebih dahulu di halaman detail transaksi untuk mengaktifkan pembayaran.
        </p>
      </div>
    </div>
  ...
  @elseif($salesOrder->status == 'cancelled')
    <div class="bg-danger/10 border border-danger/30 rounded-xl p-6 flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-sm text-silver mt-1">
          Sales Order ini telah dibatalkan. Fitur pembayaran dinonaktifkan.
        </p>
      </div>
    </div>

  {{-- KONDISI 3: FORM INPUT BAYAR --}}
  @elseif($salesOrder->remaining_balance > 0)
    <section class="bg-carbonSoft rounded-xl p-6 border border-petronas/30 shadow-lg shadow-petronas/5">
      <h2 class="text-lg font-bold text-slate-800 mb-4">Input Pembayaran Baru</h2>
      
      <form action="{{ route('sales_payments.store') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input type="number" name="amount" max="{{ $salesOrder->remaining_balance }}" step="0.01" required
                ...
                placeholder="Maks: {{ $salesOrder->remaining_balance }}">
            </div>
          ...
          <div class="md:col-span-2 lg:col-span-4">
            ...
            <input type="text" name="notes" placeholder="Keterangan pembayaran..."
              class="w-full px-4 py-2.5 rounded-lg bg-carbon border border-carbon text-silver focus:outline-none focus:border-petronas transition">
          </div>
        </div>
  ...
  @else
    <div class="bg-success/10 border border-success/30 rounded-xl p-6 flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-sm text-silver mt-1">Semua tagihan untuk pesanan ini telah dibayar lunas.</p>
      </div>
    </div>
  @endif
          ...
          @forelse($payments as $payment)
            <tr class="hover:bg-carbon transition-colors">
              ...
              <td class="px-4 py-3 text-center">
                {{-- --}}
                <form action="{{ route('sales_payments.destroy', $payment->id) }}" method="POST" onsubmit="return confirm('Hapus pembayaran ini? Saldo hutang akan kembali bertambah.')">
                  @csrf
                  @method('DELETE')
