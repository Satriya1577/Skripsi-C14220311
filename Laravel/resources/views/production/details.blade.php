    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
      <div>
        ...
        <p class="text-sm text-silver mt-1 flex flex-wrap items-center gap-2">
          <span class="bg-carbon px-2 py-0.5 rounded text-xs border border-carbonSoft">{{ $product->code }}</span>
            ...
            <form action="{{ route('forecast.approvePlan', $productionPlan->id) }}" method="POST" class="bg-carbon rounded-lg p-4 border border-carbonSoft">
              ...
              <div class="flex gap-2">
                <input type="number" 
                    ...
                    min="0" 
                    step="1"
                    class="w-32 bg-blackBase border border-carbonSoft rounded-lg px-3 py-2 text-silver text-right focus:outline-none focus:border-petronas transition focus:ring-1 focus:ring-petronas">
              ...
              @forelse($materialRecommendations ?? [] as $item)
                <tr class="hover:bg-carbon/40 transition-colors">
                  ...
                  <td class="px-4 py-3 text-right">
                    <span class=" text-warning font-bold">{{ number_format($item->qty_need, 2) }}</span> 
                    <span class="text-[10px] text-muted">{{ $item->material->unit ?? '' }}</span>
                  </td>
                  
                  <td class="px-4 py-3 text-right">
                    <span class=" text-silver">{{ number_format($item->current_stock, 2) }}</span> 
                    <span class="text-[10px] text-muted">{{ $item->material->unit ?? '' }}</span>
                  </td>
                  
                  <td class="px-4 py-3 text-right">
                    <span class=" text-silver">{{ number_format($item->purchase_otw, 2) }}</span> 
                    <span class="text-[10px] text-muted">{{ $item->material->unit ?? '' }}</span>
                  </td>
                  ...
                  <td class="px-4 py-3 text-right">
                    ...
                    @endphp
                    <span class=" font-bold {{ $shortage > 0 ? 'text-danger' : 'text-success' }}">
          ...
          <tbody class="divide-y divide-carbon/50">
            @forelse ($batches as $productionBatch)
              ...
              <tr class="hover:bg-carbon/40 transition-colors group">
                ...
                <td class="px-4 py-4 text-right font-bold {{ $realizedQty >= $targetBatchQty ? 'text-success' : 'text-warning' }}">
